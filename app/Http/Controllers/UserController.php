...
public function simForm(Request $request)
{
    ...
    if ($request->mock_status != 0)
    {
        $questions = QuizzesQuestion::where('quiz_id', (isset(Quiz::where('mock_status', $request->mock_status)->first()->id) ? Quiz::where('mock_status', $request->mock_status)->first()->id : ''))->with('questionsAnswers')->get();
//        if ($request->mock_status == 1)
//        {
//            $questions = QuizzesQuestion::where('quiz_id', (isset(Quiz::where('mock_status', 1)->first()->id) ? Quiz::where('mock_status', 1)->first()->id : ''))->with('questionsAnswers')->get();
//        } elseif ($request->mock_status == 2)
//        {
//            $questions = QuizzesQuestion::where('quiz_id', (isset(Quiz::where('mock_status', 2)->first()->id) ? Quiz::where('mock_status', 2)->first()->id : ''))->with('questionsAnswers')->get();
//        } elseif ($request->mock_status == 3)
//        {
//            $questions = QuizzesQuestion::where('quiz_id', (isset(Quiz::where('mock_status', 3)->first()->id) ? Quiz::where('mock_status', 3)->first()->id : ''))->with('questionsAnswers')->get();
//        } elseif ($request->mock_status == 4)
//        {
//            $questions = QuizzesQuestion::where('quiz_id', (isset(Quiz::where('mock_status', 4)->first()->id) ? Quiz::where('mock_status', 4)->first()->id : ''))->with('questionsAnswers')->get();
//        }
        $quiz   = Quiz::where('mock_status', $request->mock_status)->first();
        if (!$quiz)
        {
            return redirect()->back()->with('message','No Quiz or Questions found.');
        }
        $localData  = [];
        $localData['sim_qty']   = count($questions);
        $localData['sim_time']   = 0;
        $localData['sim_pass_mark']   = $quiz->pass_mark;
        ...
        return view(getTemplate() . '.user.simulator.start',[
            ...
            'is_mock'           => 1, /*free and mock end 11-1-22*/
            'quiz'      => $quiz,
        ]);
    }
...
//                 free and mock start 4-1-22
        if (Auth::user()->student_role_id == 1)
        {
            $freeQuiz = Quiz::where('is_free_quize', 1)->first();
            $questions = QuizzesQuestion::where('quiz_id', $freeQuiz->id)->with('questionsAnswers')->get();
        } else {
            $questions  = MegaQuiz::where('que_hardness', 0)->orWhere('que_hardness', 2)->with('questionsAnswers')->get(); /*que hardness 2-1-22*/
        }
//        free and mock end 4-1-22
...
//        Quiz result start
        ...
        $user = auth()->user();
        $quiz   = Auth::user()->student_role_id == 1 ? Quiz::where('is_free_quize', 1)->first() : Quiz::where('mega_quiz', 1)->first();  /*4-1-22-1*/
        $newQuizStart = QuizResult::create([
            'quiz_id' => $quiz->id,
            'student_id' => $user->id,
            'results' => '',
        ...
        return view(getTemplate() . '.user.simulator.start',[
            ...
            'newQuizStart'      => $newQuizStart,
            'quiz'      => $quiz,
            'is_mock'       => (Auth::user()->student_role_id == 1 ? 1 : ''), /*4-1-22*/
        ]);
...
//        free and mock start 4-1-22
        $freeQuiz = Quiz::where('is_free_quize', 1)->first();
//        $questions  = (Auth::user()->student_role_id == 1 ? QuizzesQuestion::where('quiz_id', (isset(Quiz::where('is_free_quize',1)->first()->id) ? Quiz::where('is_free_quize',1)->first()->id : '')) : MegaQuiz::where('category', 'like', '%' . $request->people . '%'))
        if (Auth::user()->student_role_id == 1)
        {
            $questions  = QuizzesQuestion::where('quiz_id', $freeQuiz->id)
...
//                ->orWhere('category', 'LIKE', '%' .$request->keep_mind. '%')
                ->where('que_hardness', 0)     /*que hardness 2-1-22*/
                ->orWhere('que_hardness', 1)     /*que hardness 2-1-22*/
...
//        return $questions;
        ...
        return view(getTemplate() . '.user.simulator.practice.start',[
            'que_qty'  => $que_qty,
            'randomQuestions'   => $randomQuestions,
            'quiz'   => Quiz::where('mega_quiz', 1)->first(),    /*result show 2-1-22*/
            'free_student'   => Auth::user()->student_role_id == 1 ? 1 : 0,   /*4-1-22*/
        ]);
    }
}
...
//        free and mock start 4-1-22
//        if ($user->student_role_id != 3)
        if ($user->student_role_id == 2)
//            free and mock ends 4-1-22
        {
            return \redirect()->back()->with('error', 'please upgrade your account to start this quiz');
        }
...
//        free and mock starts 4-1-22
        if ($totalmark < 75)
        {
            ...
            $subtotal = $totalmark * 1 - rand(10,15);

            $sub_total = $totalmark * 1 - rand(10,15);
            $sub_total1 = $totalmark * 1 - rand(10,15);
            $sub_total2 = $totalmark * 1 - rand(10,15);
            $sub_total3 = $totalmark * 1 - rand(10,15);
            $sub_total4 = $totalmark * 1 - rand(10,15);


            $subtotal1 = $totalmark * 1 - rand(10,15);
            $subtotal2 = $totalmark * 1 - rand(10,15);
            $subtotal3 = $totalmark * 1 - rand(10,15);
            $subtotal4 = $totalmark * 1 - rand(10,15);
        }
//        free and mock ends 4-1-22
...
//        new code ends
    ...
    public function QuizzesEdit($quiz_id)
    {
        ...
        $studentRoleTypes   = StudentQuizType::where('quiz_id',$quiz_id)->get();
        foreach ($studentRoleTypes as $x)
        {
            $types[]    =$x->student_role_id;
        }
//return $types;
        if (!empty($quiz)) {
            $data = [
...
//            test end
            // $search='keeping the team on track';
            $search='creating a high performance team';
            $count = QuizzesQuestion::Where('category', 'like', '%' . $search . '%')->pluck('title')->toArray();
                    $search1='people';
                    $question = QuizzesQuestion::select(QuizzesQuestion::raw('count(*) as count'))
                    ->Where('category', 'like', '%' . $search1 . '%')
                    ->first()
...
//return $request->all();
        ...
        if (!empty($quiz)) {
            ...
            $question_data = [
                ...
                'grade' => $data['grade']='1',
                'type' => $data['type'],
                'que_hardness' => $data['que_hardness'], /*que hardness  date-2-1-21*/
...
//        report show start 2-1-22
        if ($request->xm_type != 'practice')
        {
...
//        report show ends 2-1-22
        ...
        if ($quiz) {
            ...
            $ansIdArray = [];
            foreach ($results as $question_idx => $result)
            {
...
//                                    ->where('question_id', $question->id)
                                        ->where('question_id', (Auth::user()->student_role_id == 1 ? $question->id :$question->question_id)) /*4-1-22-1*/
            ...
            } elseif ($request->xm_type == 'practice')
            {
                $total_mark = 0;
                $status = '';
...
//                                    ->where('question_id', $question->id)
                ...
                return view(getTemplate() . '.user.simulator.practice.student_results',[
                    'total_questions'   => $request->que_qty,
...
//return $request->all();
        ...
        if (!empty($megaQuestion))
        {
            ...
            $megaData   = [
                'title' => $datax['title'],
                'grade' => $datax['grade'],
                'que_hardness' => $datax['que_hardness'], /*que hardness  date-2-1-21*/
                'image' => $datax['image'],
